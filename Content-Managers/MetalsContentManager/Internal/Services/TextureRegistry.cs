...
internal sealed class TextureRegistry : ITextureRegistry
{
    ...
    private readonly Dictionary<string, IAssetName> _assetNames =
        new(StringComparer.OrdinalIgnoreCase);

    private readonly Dictionary<string, string> _sourcePacks =
        new(StringComparer.OrdinalIgnoreCase);

    public void Register(string role, Texture2D texture, IAssetName assetName, string sourcePackId)
    {
        if (string.IsNullOrWhiteSpace(role))
            throw new ArgumentException("Asset role cannot be null or whitespace.", nameof(role));
    ...
    {
        if (!TryGetTexture(role, out Texture2D texture))
    ...
    ) => _textures.TryGetValue(role, out texture!);

    public IAssetName GetAssetName(string role)
    {
        if (!TryGetAssetName(role, out IAssetName assetName))
    ...
    public bool TryGetAssetName(string role, out IAssetName assetName) =>
        _assetNames.TryGetValue(role, out assetName!);

    public IReadOnlyCollection<string> GetAvailableRoles() =>
        _textures.Keys.OrderBy(p => p, StringComparer.OrdinalIgnoreCase).ToArray();
}