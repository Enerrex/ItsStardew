    ...
    internal static class Program
    {
        public static int Main(string[] args)
        {
            try
            {
                ...
                Type unique_ids_type = typeof(UniqueIds.Ids);
                var sections = unique_ids_type.GetNestedTypes(BindingFlags.Public | BindingFlags.Static);

                List<GeneratedProperty> constants = [];
                foreach (var section in sections)
                {
                    FieldInfo[] fields = section.GetFields
                        (BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy);

                    List<GeneratedProperty> section_constants = fields.Where
                    ...
                    constants.AddRange(section_constants);
                }
